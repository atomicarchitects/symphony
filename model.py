...
def model_run(w: WeightTuple, mace_input: MaceInput):
    ...
    focus_logits = jnp.concatenate(
        [focus_fn.apply(w.focus, features), jnp.array([0])]
    )  # (atoms)
    ...
    # get atom type
    species_logits = atom_type_fn.apply(w.atom_type, features[focus_true])
    atom_type_dist = jax.nn.softmax(species_logits)
    key, new_key = jax.random.split(key)
    ...
    return Predictions(
        ...
        species_logits,  # atom type (global)
