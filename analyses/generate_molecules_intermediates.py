...
def append_predictions(
    ...
    senders, receivers = jnp.nonzero(
        valid_edges, size=num_nodes * AVG_EDGES_PER_NODE, fill_value=-1
    )
    ...
    for d in itertools.product(range(-1, 2), repeat=3):
        shifted_rel_pos = (
            new_positions[receivers] - new_positions[senders] + np.array(d) @ cell
        )
        relative_positions = jnp.where(
            jnp.linalg.norm(shifted_rel_pos, axis=-1).reshape(-1, 1)
            < jnp.linalg.norm(relative_positions, axis=-1).reshape(-1, 1),
            shifted_rel_pos,
            relative_positions,
        )
