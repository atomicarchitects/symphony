...
def append_predictions(
    ...
    # print(padded_fragment.nodes.positions.shape, num_valid_nodes, num_nodes)
    ...
    out_frag = padded_fragment._replace(
...
#     if stop:
#         return ((padded_fragment, True), (padded_fragment, pred))
#     return ((next_padded_fragment, False), (next_padded_fragment, pred))
...
#     step_rngs = jax.random.split(rng, num=max_steps)
#     frag = init_fragment
#     stop = False
#     for rng in step_rngs:
#         ((final_padded_fragment, stop), (padded_fragments, preds)) = generate_one_step(
#             frag, stop, rng, apply_fn, cutoff
#         )
#         yield padded_fragments, preds, stop
#         frag = padded_fragments
#         stop = stop
...
def generate_molecules(
    ...
    def generator(
        ...
        step_rngs = jax.random.split(rng_gen, num=min(max_steps, max_num_atoms - frag.n_node[0]))
        stop = False
        current_frag = frag
    ...
    for seed in tqdm.tqdm(seeds, desc="Visualizing molecules"):
        ...
        for i, (padded_fragment, pred, stop) in enumerate(generator(init_fragment, rng, max_steps)):
            import pickle
            with open('padded_fragment.pkl', 'wb') as f:
                pickle.dump(padded_fragment, f)
            fragment = jraph.unpad_with_graphs(padded_fragment)  # and here lies the problem
