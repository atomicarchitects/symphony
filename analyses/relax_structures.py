...
def relax_all_structures(moldir: str, outputdir: str) -> pd.DataFrame:
    ...
    for atoms_file in os.listdir(moldir):
        if atoms_file.endswith(".xyz"):
            ...
            init_energy, final_energy, delta_energy = relax_structure(molecule, output_file, label=atoms_file.removesuffix(".xyz"))
...
def relax_structure(molecule: ase.Atoms, output_file: str, calculator: Optional[str] = None, label: Optional[str] = None) -> Tuple[float, float, float]:
    ...
    if calculator == "ORCA":
        molecule.calc = ORCA(label=label, orcasimpleinput='B3LYP 6-31G(2df,2p)', orcablock='%pal nprocs 8 end')
