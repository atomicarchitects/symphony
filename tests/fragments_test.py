    ...
    def test_fragment_generation_heavy_first(
        ...
        for frag in fragments.generate_fragments(jax.random.PRNGKey(seed), mol_graph, 5, heavy_first=heavy_first, beta_com=beta_com, mode=mode):
            ...
            if heavy_first and num_atoms <= num_heavy_atoms:
                assert jnp.all(frag.nodes.species > 0), "hydrogen atom was added to fragment before all heavy atoms were added"
            if beta_com > 0 and num_atoms == 1:
                ...
                assert jnp.abs(distances_com.min() - jnp.linalg.norm(frag.nodes.positions[0] - com)) < eps, "first atom in fragment is not closest to center of mass"
