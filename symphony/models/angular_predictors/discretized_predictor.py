...
class DiscretizedAngularPredictor(AngularPredictor):
    ...
    def coeffs(self, radius: float, conditioning: e3nn.IrrepsArray) -> e3nn.IrrepsArray:
        ...
        s2_irreps = e3nn.s2_irreps(self.max_ell, p_val=1, p_arg=-1)
        if self.apply_gate:
            irreps = e3nn.Irreps(f"{self.max_ell}x0e") + s2_irreps
        else:
            irreps = s2_irreps
        ...
        log_angular_coeffs = e3nn.IrrepsArray(
            log_angular_coeffs.irreps,
            jnp.squeeze(log_angular_coeffs.array, axis=0)
        )
