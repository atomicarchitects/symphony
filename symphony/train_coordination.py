...
@functools.partial(jax.pmap, axis_name="device", static_broadcasted_argnums=[3, 4])
def train_step(
    graphs: datatypes.Fragments,
    state: train_state.TrainState,
    rng: chex.PRNGKey,
    ...
    def loss_fn(params: optax.Params, graphs: datatypes.Fragments) -> float:
        ...
        total_loss = loss.coordination_loss(graphs=graphs, pred_logits=preds)
...
def evaluate_model(
    ...
    for split in splits:
        ...
        for graphs in device_batch(datasets[split].as_numpy_iterator()):
            ...
            batch_metrics = evaluate_step(graphs, eval_state, step_rngs)
...
def train_and_evaluate(
    ...
    net = models.create_coord_model(config)
    ...
    eval_net = models.create_coord_model(config)
