...
def generate_all_fragments(
    ...
    atomic_numbers = np.array([6, 7, 8, 9, 16, 17, 35, 53])
    molecules_as_graphs = [
    ...
    def generator():
        for graph, ndx in tqdm.tqdm(zip(molecules_as_graphs, frag_indices)):
            frags = fragments.generate_linker_fragments(
                seed,
                graph,
                ndx,
                len(atomic_numbers),
...
def main(unused_argv) -> None:
    ...
    linker_data = linker.load_linker("linker_data")  # mols, frags, frag indices, anchors

    start_index = FLAGS.start_index
    end_index = FLAGS.end_index if FLAGS.end_index != -1 else len(linker_data['mols'])
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string(
        "output_dir", "/data/NFS/potato/songk/linker_fragments/", "Output directory."
    )
