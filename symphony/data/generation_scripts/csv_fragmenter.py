...
def generate_all_fragments(
    ...
    atomic_numbers = np.arange(1, 100, dtype=np.int32)
...
def main(unused_argv) -> None:
    ...
    for split in splits:
        struct_df = pd.read_csv(os.path.join(FLAGS.input_dir, f'{split}.csv'))
        structures = struct_df['cif'].apply(lambda x: build_crystal(x))
        molecules = [
            ase.Atoms(
        ...
        output_dir = os.path.join(FLAGS.output_dir, FLAGS.mode, f"max_targets_{FLAGS.max_targets_per_graph}", split)
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string(
        "input_dir", None, "Location of input CSV files."
    )
    flags.DEFINE_integer(
