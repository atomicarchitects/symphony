...
def generation_loss(
    ...
    """
    num_graphs, num_nodes_for_multifocus, num_targets = graphs.globals.target_position_mask.shape
    ...
    def focus_and_atom_type_loss() -> jnp.ndarray:
        ...
        stop_targets = graphs.globals.stop.astype(jnp.float32).reshape((-1,))
    ...
    def position_loss() -> jnp.ndarray:
        ...
        assert graphs.globals.target_positions.shape == (num_graphs, num_nodes_for_multifocus, num_targets, 3), (
        ...
        assert position_logits.shape == (num_graphs, num_nodes_for_multifocus, num_targets), (
        ...
        assert target_position_mask.shape == (num_graphs, num_nodes_for_multifocus, num_targets)

        loss_position = -position_logits
        loss_position = jnp.where(target_position_mask, loss_position, 0)
        loss_position = loss_position.sum(axis=(-2, -1))
        num_valid_targets = jnp.maximum(1, target_position_mask.sum(axis=(-2, -1)))
        ...
        # jax.debug.print("mask={x}", x=graphs.globals.target_position_mask)
