...
def generation_loss(
    ...
    """
    _, num_nodes_for_multifocus, num_targets = graphs.globals.target_positions_mask.shape
    ...
    def focus_and_atom_type_loss() -> jnp.ndarray:
        ...
        assert stop_targets.shape == (num_graphs,), stop_targets.shape
    ...
    def position_loss() -> jnp.ndarray:
        ...
        def process_logits(logits):
            ...
            assert target_positions_mask.shape == (num_graphs, num_nodes_for_multifocus, num_targets)

            loss = -position_logits
            loss = jnp.where(target_positions_mask, loss, 0)
            loss = loss.sum(axis=(-2, -1))
            num_valid_targets = jnp.maximum(1, target_positions_mask.sum(axis=(-2, -1)))
