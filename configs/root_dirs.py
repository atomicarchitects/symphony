...
def get_root_dir_tf(dataset: str, fragment_logic: str) -> Optional[str]:
    ...
    if dataset == "qm9":
        ...
        if hostname == "potato.mit.edu":
            if max_targets_per_graph:
                return f"/data/NFS/potato/songk/qm9_fragments_multifocus_mini/{fragment_logic}/max_targets_{max_targets_per_graph}"
    ...
    if dataset == "tmqm":
        if hostname == "potato.mit.edu":
            if max_targets_per_graph:
                return f"/data/NFS/potato/songk/tmqm_fragments_multifocus/{fragment_logic}/max_targets_{max_targets_per_graph}"
