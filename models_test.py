...
class ModelsTest(parameterized.TestCase):
    ...
    def test_segment_softmax_with_zero(self):
        logits = jnp.asarray([jnp.log(2), jnp.log(2), jnp.log(1), jnp.log(3), jnp.log(2)])
        n_node = jnp.asarray([2, 3])
        segment_ids = models.get_segment_ids(n_node, num_nodes=5, num_graphs=2)
        
        probs, zero_probs = models.segment_softmax_with_zero(logits, segment_ids, num_segments=2)
